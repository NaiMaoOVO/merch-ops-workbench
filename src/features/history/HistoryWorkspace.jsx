...
export default function HistoryWorkspace({ storage: storageProp }) {
  ...
  const localUsage = useMemo(() => {
    try { return collectFullBackup(storage ?? undefined).entries.reduce((total, entry) => total + entry.key.length + entry.value.length, 0); } catch { return 0; }
  }, []);
  ...
  return <main className="history-workspace" data-testid="history-workspace">
    ...
    <section className="history-list">{visible.map((project) => <article className={`history-project glass-card ${project.archived ? 'is-archived' : ''}`} key={project.id}><div className="history-project-main"><div className="history-project-title"><h2>{project.name}</h2>{project.archived && <span className="soft-status">已归档</span>}</div><div className="history-project-meta"><span>{project.period ?? '未设置周期'}</span><span>更新于 {String(project.updatedAt ?? '').slice(0, 10) || '—'}</span><span>{project.selectedTables?.length ?? 0} 张数据表</span>{project.analysisSummary?.totals?.salesAmount > 0 && <span>销售额 ¥{Math.round(project.analysisSummary.totals.salesAmount).toLocaleString()}</span>}{project.analysisSummary?.diagnostics && <span>异常 {project.analysisSummary.diagnostics.filter((item) => item.status !== '已解决' && item.status !== '已忽略').length} 条待处理</span>}{project.schemaVersion >= 2 && <span>本地占用约 {formatBytes(estimateProjectBytes(project))}</span>}</div><div className="history-progress"><span style={{ width: `${Math.round((project.progress ?? 0) * 100)}%` }} /></div></div><div className="history-project-actions"><label className="history-compare-check"><input type="checkbox" checked={comparisonIds.includes(project.id)} onChange={() => toggleComparison(project.id)} />对比</label><button className="icon-button" aria-label={`归档${project.name}`} onClick={() => toggleArchive(project)}><Archive size={16} /></button><button className="text-button" onClick={() => { const copy = cloneProject(project); persist([copy, ...projects]); setMessage('已创建项目副本。'); }}>复制</button>{<button className="text-button" onClick={() => { if (window.confirm(`删除项目「${project.name}」及其本地草稿？该操作不可恢复。`)) { removeAnalysisProject(project.id, storage, { includeReportDrafts: true }); setProjects((current) => current.filter((item) => item.id !== project.id)); setComparisonIds((current) => current.filter((id) => id !== project.id)); setMessage(`已删除「${project.name}」。`); } }}>删除</button>}</div></article>)}</section>
  </main>;
}
